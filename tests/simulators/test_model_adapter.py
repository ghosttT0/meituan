...
def test_http_model_adapter_builds_openai_messages_payload() -> None:
    adapter = HttpModelAdapter(endpoint="https://api.deepseek.com/v1")
    adapter.protocol_mode = "openai"
    adapter.model = "deepseek-chat"
    payload = adapter.build_payload(
        session_id="session_1",
        history=[
            {"speaker": "user", "text": "为什么必须这样？"},
            {"speaker": "assistant", "text": "因为这次主要是确认安排。"},
        ],
        task_instruction_text="请先确认用户身份，再确认收货时间。",
    )
    ...
    assert payload["messages"][0]["content"] == "请先确认用户身份，再确认收货时间。"
